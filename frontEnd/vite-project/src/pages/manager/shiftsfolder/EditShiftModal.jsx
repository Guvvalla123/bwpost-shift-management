      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-600 px-6 py-5 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Edit Shift</h2>
              <p className="text-blue-200 text-xs truncate max-w-[220px]">{editingShift.shiftTitle}</p>
            </div>
          </div>
          <button
            onClick={() => setEditingShift(null)}
        ...
        {/* Form */}
        <form onSubmit={onUpdateHandler} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Start Time">
              <input
                type="datetime-local"
                name="shiftStartTime"
                value={editingShift.shiftStartTime?.slice(0, 16)}
                onChange={onEditChange}
                className={inputCls}
                required
              />
            ...
            <Field label="End Time">
              <input
                type="datetime-local"
                name="shiftEndTime"
                value={editingShift.shiftEndTime?.slice(0, 16)}
                onChange={onEditChange}
                className={inputCls}
                required
              />
