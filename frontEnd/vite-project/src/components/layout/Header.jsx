...
const navLinkClass =
  "text-sm font-medium text-slate-600 transition-colors hover:text-[#1B3F8B] dark:text-slate-300 dark:hover:text-[#93C5FD]";
...
const btnIcon =
  "inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white/80 text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800";
...
export default function Header() {
      ...
      {[
        ...
        <Link
          ...
          className={cn(
            ...
            mobile && "rounded-lg px-3 py-2.5 hover:bg-slate-100 dark:hover:bg-slate-800"
          )}
        >
          {label}
        </Link>
  ...
  const MobilePanel = ({ children }) =>
    mobileOpen ? (
      <>
        ...
        />
        <div
          className="fixed inset-y-0 right-0 z-[60] flex w-[min(100vw-2rem,20rem)] flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 md:hidden"
          role="dialog"
          aria-modal="true"
          aria-labelledby="mobile-menu-title"
        >
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
            <h2 id="mobile-menu-title" className="text-lg font-semibold text-slate-900 dark:text-white">
              Menu
            </h2>
            <button
              type="button"
  ...
  return (
    <header className="fixed left-0 top-0 z-40 w-full border-b border-slate-200/80 bg-white/90 shadow-sm backdrop-blur-md dark:border-slate-800 dark:bg-slate-950/90 dark:shadow-slate-950/50">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
        <Link
          to="/"
          className="shrink-0 text-xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-2xl"
          onClick={() => setMobileOpen(false)}
        >
