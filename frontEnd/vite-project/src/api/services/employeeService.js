...
//
// HOW TO USE:
//   import { getAllEmployees, addEmployee } from "./employeeService";
...
export async function updateEmployee(employeeId, employeeData) {
  const response = await API.put(`/api/manager/shifts/employees/${employeeId}`, employeeData);
  return response.data;
}
...
export async function removeEmployee(employeeId) {
  const response = await API.delete(`/api/manager/shifts/employees/${employeeId}`);
  return response.data;
}
...
export async function getEmployeeAttendance(employeeId) {
  const response = await API.get(`/api/manager/shifts/employees/${employeeId}/attendance`);
